 ...
 * @Description: 文档页面
 * @Author: yinlele
 * @Date: 2022-02-07 18:18:16
 * @LastEditTime: 2022-02-08 14:16:16
 * @LastEditors: yinlele
 * @FilePath: /y-ui/src/views/doc/index.vue
-->
<template>
  <div class="layout">
    <Topnav class="nav" />
    <div class="content">
      <aside :style="asideStyle">
        <div class="menu">
          <div
            class="menu-group"
            v-for="(itemx, key) in docMenus"
            :key="key"
          >
            <span class="menu-group-title text-overflow">
              {{ key }}{{ key !== "文档" ? `（${itemx.length}）` : "" }}
            </span>
            <router-link
...
const toggleAside = () => {
  (asideVisible as Ref<boolean>).value = !(asideVisible as Ref<boolean>)
.value;
};
const asideStyle = computed(() => {
  return { left: (asideVisible as Ref<boolean>).value ? "0px" : "-272px" };
});
const toggleAsidStyle = computed(() => {
  return {
    left: (asideVisible as Ref<boolean>).value ? "272px" : "0px",
    transform: (asideVisible as Ref<boolean>).value
      ? "rotate(180deg) translateX(50%)"
      : "rotate(0deg) translateX(50%)",
  };
});
const mainStyle = computed(() => {
  return (clientWidth as Ref<number>).value < 500
    ? { "padding-left": "24px" }
    : { "padding-left": (asideVisible as Ref<boolean>).value ? "302px" : "60px" };
});
</script>
<style lang="scss">
.layout {
